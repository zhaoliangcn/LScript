...
void CFileView::UpdateOpenedFile(const wchar_t * filePathName, BOOL bOpen)
{
	if (bOpen)
	{
	...
	else
	{
		...
		while (child)
		{
			...
			{
				m_wndFileView.DeleteItem(child);
			}
			child = m_wndFileView.GetNextSiblingItem(child);
		}
	}
	m_wndFileView.Expand(m_wndFileView.GetRootItem(), TVE_EXPAND);
}
...
{
	HTREEITEM hRoot = m_wndFileView.InsertItem(_T("LScript脚本文件"), 0, 0);
